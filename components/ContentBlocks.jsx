  ...
  return text.map((value, i) => {
    ...
    return (
      <span
        ...
        className={[
          bold ? 'font-bold' : '',
          code ? 'bg-gray-400 p-1 font-mono text-sm rounded-md' : '',
          italic ? 'italic' : '',
...
export const Heading = ({ text, level }) => {
  switch (level) {
    case 'heading_1':
      return (
        <h1 className="font-bold text-3xl md:text-5xl tracking-tight my-2 text-gray-300">
          {text[0].text.content}
        </h1>
      )
    case 'heading_2':
      return (
        <h2 className="font-bold text-2xl md:text-3xl tracking-tight my-2 text-gray-300">
          {text[0].text.content}
        </h2>
      )
    case 'heading_3':
      return (
        <h3 className="font-bold text-lg md:text-xl tracking-tight my-2 text-gray-300">
          {text[0].text.content}
        </h3>
      )
    default:
